...
#include <vector>
#include "my_ros_package/SimplePointCloud.h"  // 自定义点云结构体的头文件
#include "my_ros_package/GridMap.h"
...
public:
     ...
     * @return 转换得到的点云
     */
    SimplePointCloud ConvertToPointCloud(const TCP_NX2PC_MSG* disparity_data);
    
    /**
     * @brief 更新栅格地图数据。
     * @param disparity_data 包含视差数据的 TCP_NX2PC_MSG 结构体指针。
     */
    void UpdateGridMap(const TCP_NX2PC_MSG* disparity_data);
